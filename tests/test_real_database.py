...
class TestRealDatabase:
    """Test functionality with real macOS 15 database."""
    
    def test_database_version_detection(self, database_with_connection, database_metadata):
    ...
    def test_database_uuid_extraction(self, database_with_connection, database_metadata):
    ...
    def test_accounts_extraction(self, database_with_connection, database_metadata):
        ...
        on_my_mac_account = next((acc for acc in accounts if acc.name == database_metadata["account_name"]), None)
        assert on_my_mac_account is not None
        assert on_my_mac_account.name == database_metadata["account_name"]
    
    def test_folders_extraction_and_hierarchy(self, database_with_connection, sample_folders_data):
        ...
        for expected in sample_folders_data["expected_folders"]:
            ...
            assert actual_path == expected["path"], f"Expected path '{expected['path']}', got '{actual_path}'"
            
            # Check parent relationship
            if expected["parent"] is None:
                assert folder.is_root(), f"Folder '{expected['name']}' should be root folder"
                assert folder.parent_id is None
            else:
                assert not folder.is_root()
                assert folder.parent_id is not None
                parent_folder = folder.get_parent(folders_dict) 
                assert parent_folder.name == expected["parent"]
    
    def test_notes_extraction_basic(self, database_with_connection, database_metadata):
    ...
    def test_specific_notes_content(self, database_with_connection, sample_notes_data):
        ...
        assert tagged_note.is_password_protected == sample_notes_data["tagged_note"]["password_protected"]
        assert tagged_note.applescript_id == sample_notes_data["tagged_note"]["applescript_id"]
        ...
        assert protected_note.is_password_protected == sample_notes_data["protected_note"]["password_protected"]
        assert protected_note.applescript_id == sample_notes_data["protected_note"]["applescript_id"]
        ...
        assert subfolder_note.applescript_id == sample_notes_data["subfolder_note"]["applescript_id"]
    
    def test_note_content_extraction(self, database_with_connection):
        ...
        assert len(notes_with_content) > 0, "No notes have content - content extraction may be failing"
    ...
    def test_folder_path_reconstruction(self, database_with_connection):
...
class TestRealDatabaseWithParser:
    """Test AppleNotesParser with real database."""
    
    def test_parser_initialization_with_real_db(self, real_database):
        """Test parser can initialize with real database."""
        parser = AppleNotesParser(real_database)
        assert parser.database_path.exists()
    
    def test_parser_data_loading(self, real_database, database_metadata):
    ...
    def test_tag_functionality(self, real_database, sample_notes_data):
    ...
    def test_password_protection_detection(self, real_database):
    ...
    def test_search_functionality(self, real_database):
    ...
    def test_export_functionality(self, real_database):
